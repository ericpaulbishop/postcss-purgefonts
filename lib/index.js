...
function initOptions(pluginOptions, postcssOptions) {
  ...
  for (let dki = 0; dki < defaultKeys.length; dki++) {
    ...
    if (options[key] == null) {
      //format_load_order should be internal-only value
  ...
  options['absolute_to'] = options['relative_to'].charAt(0) == '/' ? options['relative_to'] : postcssTo + "/" + options['relative_to']; //cache businting must be one of three valid options, if it doesn't match set it to none

  options['cache_busting'] = options['cache_busting'].toLowerCase();
  options['cache_busting'] = options['cache_busting'] == 'file' || options['cache_busting'] == 'query' ? options['cache_busting'] : "none"; //other internal global constants 
...
function getTruetypeCodePoints(path) {
  ...
  try {
    ...
    let font = fonteditor.Font.create(inputBuffer, {
      type: 'ttf'
    });
...
function purgeTruetypeGlyphs(srcPath, destPath, glyphs, saveHinting) {
  try {
    ...
    let inputBuffer = fs.readFileSync(srcPath); // read font data

    let fontObject = fonteditor.Font.create(inputBuffer, {
      type: 'ttf',
      // support ttf, woff, woff2, eot, otf, svg
      subset: codePoints,
      // only read glyphs with these codepoints 
      hinting: saveHinting,
      // save font hinting
      compound2simple: false,
      // transform ttf compound glyf to simple
      inflate: null,
...
async function convertFromTruetype(srcPath, destPath, destFormat) {
  ...
  try {
    ...
    let fontObject = fonteditor.Font.create(rawInput, {
      type: 'ttf'
    });
    let outputBuffer = Buffer.from(fontObject.write({
      type: convertModule
    }));
    fs.writeFileSync(destPath, outputBuffer);
  } catch (err) {
...
async function analyzeFont(options, glyphs, fontFiles, fontFamily) {
  ...
  debugLog(extensionToFontPath); //find ttf source, converting another format to ttf if necessary
  ...
  if (analysis['ttf_src_path'] == null || ignoreFontMap[fontFamily]) {
    ...
    let fontCodePointMap = createMap(fontGlyphCodePoints); //debugLog("size of character set = " + fontGlyphCodePoints.length)
    ...
    if (glyphsToKeep.length == 0 && options.ignore_all_on_zero_matching_glyphs) {
      ...
      analysis['action'] = 'ignore';
    } else if (glyphsToKeep.length == 0 && options.preserve_all_on_zero_matching_glyphs || preserveFontMap[fontFamily] || processOnlyFontMap != null && !processOnlyFontMap[fontFamily]) {
...
async function absolutePathToUrl(options, path, format, isBareEot) {
  ...
  try {
    if (options.cache_busting == "query") {
      ...
      cacheBuster = "?fonthash=" + (await hasha.fromFileSync(path, {
        algorithm: 'sha256'
      }).substr(-8)); //last 8 characters of sha256 hash
    }
  } catch (e) {
    debugLog("HASH ERROR : " + e);
  }
...
async function processFontAndGenerateSrcs(options, fontFiles, fontAnalysis, oldSrcs) {
  ...
  if (fontAnalysis['action'] == 'ignore') {
    ...
    if (fs.existsSync(finalTtfDest) && options.cache_busting == "file") {
      let cacheBustingTtfDest = destRoot + "-" + (await hasha.fromFileSync(finalTtfDest, {
        algorithm: 'sha256'
      }).substr(-8)) + ".ttf";
      ...
      for (let fooi = 0; fooi < options['format_output_order'].length; fooi++) {
        ...
        if (fmt.fmt != 'truetype') {
          ...
          if (fs.existsSync(dest) && options.cache_busting == "file") {
            let cacheBustingDest = destRoot + "-" + (await hasha.fromFileSync(dest, {
              algorithm: 'sha256'
            }).substr(-8)) + "." + fmt.ext;
...
async function updateFontRules(cssRoot, options, glyphs) {
  ...
  for (let frtpi = 0; frtpi < fontRulesToProcess.length; frtpi++) {
    ...
    debugLog(glyphs); //debugLog(fontFiles)
